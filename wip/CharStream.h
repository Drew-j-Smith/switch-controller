...
template<size_t frameSize>
int CharStream<frameSize>::hexToInt(char t){
    if(isdigit(t))
        return t - '0';
    else
...
template<size_t frameSize>
void CharStream<frameSize>::save(const std::string & filename, const bool asHex = true){
...
template<size_t frameSize>
void CharStream<frameSize>::print(std::ostream & out, const bool asHex = true){
    if (asHex){
