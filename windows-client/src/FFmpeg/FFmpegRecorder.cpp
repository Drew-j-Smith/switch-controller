...
static std::tuple<
    ...
    std::map<int, std::unique_ptr<FFmpegDecoder>>>
openStream(const std::string &inputFormatStr, const std::string &deviceNameStr,
    ...
    return {std::move(formatContext), std::move(decoders)};
}
