...
bool InputEventCollection::loadEventType(
    ...
    if (it->second.get("type", "") == "SF Keyboard Input") {
        inputEvents.push_back(
            std::make_shared<SfKeyboardInputEvent>(it->second));
        return true;
    }
    if (it->second.get("type", "") == "SF Digital Joystick Input") {
        inputEvents.push_back(
            std::make_shared<SfJoystickDigitalInputEvent>(it->second));
        return true;
    }

    std::cerr << "Unkown type in macro collection \""
              << it->second.get("type", "") << "\".\n";
    return false;
}

void InputEventCollection::addEventModifiers(
    const boost::property_tree::ptree::const_iterator &it) {
    if (it->second.get("negated", false)) {
        inputEvents.back() =
            std::make_shared<InputEventInverter>(inputEvents.back());
    }
    if (it->second.get("toggle", false)) {
        inputEvents.back() =
            std::make_shared<InputEventToggle>(1000, inputEvents.back());
    }
    if (it->second.get("turbo", false)) {
        inputEvents.back() = std::make_shared<InputEventTurbo>(
            it->second.get("turbo loop", 500), inputEvents.back());
    }
}
...
InputEventCollection::makeShared(const boost::property_tree::ptree &tree,
                                 Factory<InputEvent> &factory) const {
    std::vector<std::shared_ptr<InputEvent>> newInputEvents;
    boost::property_tree::ptree childTree = tree.get_child("events");
    for (auto event : childTree) {
        newInputEvents.push_back(factory.generateObject(event.second));
    }
