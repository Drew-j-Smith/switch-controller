...
public:
    ...
    InputEventTurbo(const boost::property_tree::ptree &tree,
        ...
        toggleEvent = std::make_shared<InputEventToggle>(
            loopTime, event); // TODO this should not be calling make_shared
    }
    ...
    makeShared(const boost::property_tree::ptree &tree,
        ...
        boost::property_tree::ptree childTree = tree.get_child("event");
        int loopTime = tree.get<int>("loop time");
        return std::make_shared<InputEventTurbo>(
            loopTime, factory.generateObject(childTree));
    }
