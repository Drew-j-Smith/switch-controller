...
DeciderCollection::DeciderCollection(const boost::property_tree::ptree &tree) {
    deciderCollections.push_back(
        std::make_shared<ImageProcessingDeciderCollection>(tree));
    deciderCollections.push_back(
        std::make_shared<SoundDeciderCollection>(tree));
}

std::map<std::string, std::shared_ptr<Decider>>
DeciderCollection::generateMap() const {
